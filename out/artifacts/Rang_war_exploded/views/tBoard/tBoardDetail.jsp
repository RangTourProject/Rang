...
<div class="container"> <!-- container 시작 -->
	...
	<!--       <img class="media-object" src="" alt="..."> --><!-- 프로필 받아오기 미구현 --> 
									...
									<div class="media-body">
										<h3 class="media-heading">&nbsp;&nbsp;${member.nickName}<span class="atFont"></span></h3>
								    	&nbsp;&nbsp;&nbsp;
										<button class="btn"><i class="fa fa-user-plus" aria-hidden="true"></i> 팔로팔로미</button>
									</div>
								</div>
							</div>
	<!-- 왼쪽 개인프로필 만들기  끝-->
							<div class="findPeople">
								<p><i class="fa fa-user" aria-hidden="true"></i> &nbsp;&nbsp;버전 0.001<p>
							</div>
					</div>
				</div>
			</div>
					...
					<tbody>
				  		<tr>
							...
							<td align=left style="cols:40">
								...
								<!-- 대댓글 창 -->
								<div class="media-body">
									<h5 class="media-heading">${member.nickName}<span class="userFont">${tbco.tcdate}</span></h5>
									<textarea class="reply-content" cols="40" rows="3"
								...
								<button type="button" class="updateBtn btn btn-primary " 
										onclick="updateReply(this);">고침</button>
										
								<button type="button" class="updateConfirm btn btn-primary"
										onclick="updateConfirm(this);"
										style="display:none;" >고침끗</button> &nbsp;&nbsp;
										
								<button type="button" class="deleteBtn btn btn-danger"
										onclick="deleteReply(this);">지움</button> &nbsp;&nbsp;		
								</c:if>
								<c:if test="${ tbco.tclevel lt 3 }">
								<input type="hidden" name="userno" value="${member.userNo}"/>
								<input type="hidden" name="reftcno" value="${tbco.tcno}" />
								<input type="hidden" name="tclevel" value="${tbco.tclevel}" />
								<button type="button" class="insertBtn btn btn-danger" 
										 onclick="reComment(this);">댓글</button>&nbsp;&nbsp;
